...
static inline long ptrace_traceme() { return ptrace(PTRACE_TRACEME, 0, NULL, NULL); }
...
static inline long ptrace_kill(pid_t pid) { return ptrace(PTRACE_KILL, pid, NULL, NULL); }
...
static inline void vm_simple(bool read, /* 1 read, 0 write */
                             pid_t cpid, unsigned long addr, ssize_t n, void *data) {
...
static inline void vm_write_simple(pid_t cpid, unsigned long addr, ssize_t n,
                                   void *data) {
...
static inline uint16_t read_semaphore_ptrace(pid_t cpid, unsigned long addr,
                                             signed long *data) {
...
static inline void write_semaphore_ptrace(pid_t cpid, unsigned long addr,
  ...
  if (verbose)
    fprintf(stderr, "new data at %lx: %lx\n", addr, new_data);
...
static inline uint16_t read_semaphore_vm(pid_t cpid, unsigned long addr) {
  uint16_t semaphore = 0;
  vm_read_simple(cpid, addr, 2, &semaphore);
  return semaphore;
}

static inline void write_semaphore_vm(pid_t cpid, unsigned long addr,
                                      uint16_t new_semaphore) {
...
static inline uint16_t read_semaphore_self(unsigned long addr) {
  ...
  if (verbose)
    fprintf(stderr, "semaphore at %lx = %x\n", addr, semaphore);
  return semaphore;
}

static inline void write_semaphore_self(unsigned long addr, uint16_t new_semaphore) {
  *((uint16_t *)addr) = new_semaphore;
}

static inline bool is_enabled_opcode(uint8_t byte, pid_t cpid, unsigned long addr) {
  switch (byte) {
  case CMP_OPCODE:
    return false;
  case CALL_OPCODE:
    return true;
  default:
    raise_error("Unexpected instruction opcode %x at %lx for pid %d", byte, addr, cpid);
  }
...
static inline uint8_t read_opcode_self(unsigned long addr) {
  ...
  if (verbose)
    fprintf(stderr, "cur at %lx: %x\n", addr, opcode);
  return opcode;
}

static inline void mprotect_self(bool for_writing, unsigned long pagestart,
  ...
  if (mprotect((void *)pagestart, pagesize, prot)) {
    raise_error("modify_probe in self"
...
static inline void write_opcode_self(unsigned long addr, uint8_t opcode) {
  ...
  if (sigsetjmp(sigbus_jmp_buf, 1) == 0) {
    ...
    *((uint8_t *)addr) = opcode;
    ...
    raise_error("modify_probe in self: text segment write failed to reserve a backing "
                "page and raised SIGBUS. (addr=%lx)",
                addr);
  }
}

static inline uint8_t read_opcode_ptrace(pid_t cpid, unsigned long addr,
                                         unsigned long *data) {
  uint8_t opcode = 0;
  errno = 0;
  *data = ptrace_get_text(cpid, (void *)addr);
  ...
  if (errno != 0)
    raise_error("read_opcode of probe in pid %d: "
                ...
                cpid, addr, errno);
  if (verbose)
    fprintf(stderr, "cur at %lx: %x with data = %lx\n", addr, opcode, *data);
  return opcode;
}

static inline uint8_t read_opcode_vm(pid_t cpid, unsigned long addr) {
  uint8_t opcode = 0;
  vm_read_simple(cpid, addr, 1, &opcode);
  return opcode;
}

static inline void write_opcode_ptrace(pid_t cpid, unsigned long addr, uint8_t opcode,
                                       unsigned long data) {
...
static inline void write_semaphore(mode mode, pid_t cpid, unsigned long addr,
                                   uint16_t sem) {
  switch (mode) {
  case Mode_self:
    write_semaphore_self(addr, sem);
    return;
  case Mode_ptrace:
    write_semaphore_ptrace(cpid, addr, sem);
    return;
  case Mode_vm:
    write_semaphore_vm(cpid, addr, sem);
    return;
  default:
    raise_error("mode undefined %d\n", mode);
  }
}

static inline uint16_t read_semaphore(mode mode, pid_t cpid, unsigned long addr) {
  signed long data = 0;
  switch (mode) {
  case Mode_self:
    return read_semaphore_self(addr);
  case Mode_vm:
    return read_semaphore_vm(cpid, addr);
  case Mode_ptrace:
    // data is ignored
...
static inline uint8_t read_opcode(mode mode, pid_t cpid, unsigned long addr,
                                  unsigned long *data) {
  switch (mode) {
  case Mode_self:
    return read_opcode_self(addr);
  case Mode_vm:
    return read_opcode_vm(cpid, addr);
  case Mode_ptrace:
    return read_opcode_ptrace(cpid, addr, data);
  default:
    raise_error("mode undefined %d\n", mode);
  }
  __builtin_unreachable();
...
static inline void write_opcode(mode mode, pid_t cpid, unsigned long addr, uint8_t op,
                                unsigned long data) {
  switch (mode) {
  case Mode_self:
    write_opcode_self(addr, op);
    return;
  case Mode_vm:
    raise_error("cannot vm_process_write at 0x%x: opcode\n", addr);
  case Mode_ptrace:
    write_opcode_ptrace(cpid, addr, op, data);
    return;
  default:
    raise_error("mode undefined %d\n", mode);
  }
...
static inline void modify_opcode(mode mode, pid_t cpid, unsigned long addr, bool enable) {
...
CAMLprim value probes_lib_start_ptrace(value v_argv) {
  ...
  char **argv = (char **)malloc((argc + 1 /* for NULL */) * sizeof(char *));
  for (int i = 0; i < argc; i++) {
    argv[i] = strdup(String_val(Field(v_argv, i)));
    if (verbose)
      fprintf(stderr, "start: argv[%d]=%s\n", i, argv[i]);
  }
  argv[argc] = NULL;
...
CAMLprim value probes_lib_read_semaphore(value v_mode, value v_pid, value v_address) {
...
CAMLprim value probes_lib_write_semaphore(value v_mode, value v_pid, value v_addresses,
                                          value v_sem) {
...
CAMLprim value probes_lib_write_probes(value v_mode, value v_pid, value v_pagestart,
                                       value v_pagesize, value v_updates) {
